...
try:
    ...
    # If not, install it
    print("APScheduler not found. Installing...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "APScheduler"])
        print("APScheduler installed successfully.")
    except subprocess.CalledProcessError:
        print("Failed to install APScheduler. The bot may not function correctly.")
# --- End of auto-installer ---
...
# --- Configuration ---
TOKEN = '8087380741:AAHdksoiNmIuXGGoC5596rQxH218WCE7sXs' # Replace with your actual token
OWNER_ID = 7507183871 # Replace with your Owner ID
ADMIN_ID = 7507183871 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@uditanshu_sarkar' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 't.me/hostify_network' # Replace with your update channel link
...
def logs_bot_callback(call):
    try:
        ...
        try:
            ...
            # A safe character limit for the log content, leaving room for the message header
            SAFE_CONTENT_LIMIT = 3800 

            if len(log_content) > SAFE_CONTENT_LIMIT:
               # Truncate the log content and add a note that it was shortened
               log_content = f"... (log truncated)\n" + log_content[-SAFE_CONTENT_LIMIT:]

            if not log_content.strip(): 
               log_content = "(No visible content in log)"
